...
/**
 * Deletes the Anthropic API key from the macOS Keychain
 *
 * @returns Promise resolving to true if deleted, false if not found or error
 */
...
/**
 * Checks if an API key is stored in the macOS Keychain
 *
 * @returns Promise resolving to true if a key exists
 */
export async function hasApiKeyInKeychain(): Promise<boolean> {
  const key = await getApiKeyFromKeychain();
  return key !== null;
}

/**
 * Loads the API key from keychain into the environment if not already set
 *
 * This should be called early in the application startup to make the
 * keychain-stored key available via process.env.ANTHROPIC_API_KEY
 *
 * @returns Promise resolving to true if key was loaded from keychain
 */
export async function loadApiKeyFromKeychain(): Promise<boolean> {
